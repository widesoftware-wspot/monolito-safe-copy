...
class PostAccessPointsGroupsNewHandler implements AnalyticsHandler
{
	const EVENT_CATEGORY    = 'Pontos de Acesso (Grupos)';
	const EVENT_NAME        = 'Cadastrar Grupo AP';
	...
	/**
	 * PostAccessPointsGroupsNewHandler constructor.
	...
	public function build(Request $request, $extra = null)
	{
		...
		$mapperFields    = $mapper['routes']['access_points_groups_new']['fields'];
		$postFields      = $request->request->all();
        $eventProperties = [];

		foreach ($mapperFields as $key=>$event) {
			$value = $this->getFieldValueByKey($postFields['access_point_group'], $key);
	...
	{
		return (array_key_exists($key, $fields) && $fields[$key] == '1') ? 'Sim' : 'Não';
	}
}
