...
class PostConfigurationsEditHandler implements AnalyticsHandler
{
	const EVENT_CATEGORY    = 'Pontos de Acesso (Grupos)';
	const EVENT_NAME        = 'Configuracoes';
	...
	/**
	 * PostConfigurationsEditHandler constructor.
	...
	public function build(Request $request, $extra = null)
	{
		...
		$mapperFields    = $mapper['routes']['configurations_edit']['fields'];
		$postFields      = $request->request->all();
        $eventProperties = [];

		foreach ($mapperFields as $key=>$event) {
			$value = $this->getFieldValueByKey($postFields['setup'], $key);
	...
	{
		return (array_key_exists($key, $fields) && $fields[$key]['value'] == '1') ? 'Sim' : 'Não';
	}
}
