...
class PostApiEgoiNewHandler implements AnalyticsHandler
{
	const EVENT_CATEGORY    = 'Integrações';
	const EVENT_NAME        = 'Integracao Egoi Cadastrar';
	...
	/**
	 * PostApiEgoiNewHandler constructor.
	...
	public function build(Request $request, $extra = null)
	{
		...
		$mapperFields    = $mapper['routes']['api_egoi_new']['fields'];
		$postFields      = $request->request->all();
        $eventProperties = [];

		foreach ($mapperFields as $key=>$event) {
			$value = $this->getFieldValueByKey($postFields['wspot_api_egoi'], $key);
