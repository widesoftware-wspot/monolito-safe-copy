...
class PostAccessCodeNewHandler implements AnalyticsHandler
{
	const EVENT_CATEGORY    = 'Código de Acesso ';
	const EVENT_NAME        = 'Cadastrar Codigo Acesso';
	...
	/**
	 * PostAccessCodeNewHandler constructor.
	...
	public function build(Request $request, $extra = null)
	{
		...
		$mapperFields    = $mapper['routes']['access_code_new']['fields'];
		$postFields      = $request->request->all();
        $eventProperties = [];

		foreach ($mapperFields as $key=>$event) {
			$value = $this->getFieldValueByKey($postFields['wspot_access_code'], $key);
	...
	{
		if ($key == 'type') {
			return ($fields[$key] == 'predefined') ? 'Pré Definido' : 'Aleatórios';
		}

		if ($key == 'enable') {
			return ($fields[$key] == '1') ? 'Ativo' : 'Inativo';
		}

		if ($key == 'step') {
			return ($fields[$key] == 'login') ? 'Primeira Tela' : 'Após o cadastro';
		}
