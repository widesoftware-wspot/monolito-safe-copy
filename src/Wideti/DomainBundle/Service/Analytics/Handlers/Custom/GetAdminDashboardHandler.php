...
class GetAdminDashboardHandler implements AnalyticsHandler
{
	const EVENT_CATEGORY    = 'Dashboard';
	const EVENT_NAME        = 'Visualizar Dashboard';
	const TAB               = 'Visão Geral';
	const LOGIN             = 'Realizou Login';
	...
	/**
	 * GetAdminDashboardHandler constructor.
	...
	{
		if (empty($postFields['filter'])) return null;

		$filter = $postFields['filter']['filter'];
		$value = 'Todo período';
