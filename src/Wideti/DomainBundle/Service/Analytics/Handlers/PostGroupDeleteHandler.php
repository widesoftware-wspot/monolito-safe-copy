...
class PostGroupDeleteHandler implements AnalyticsHandler
{
	const EVENT_CATEGORY    = 'Grupo de Visitantes';
	const EVENT_NAME        = 'Remover Grupo Visitantes';
	...
	/**
	 * GetGroupDeleteHandler constructor.
	...
	public function build(Request $request, $extra = null)
	{
		...
		foreach ($mapperFields as $key=>$event) {
			if ($key === 'apsAndGroups') continue;
            $eventProperties[$event] = 'Inativo';
		}
