...
class PostSegmentationNewHandler implements AnalyticsHandler
{
	const EVENT_CATEGORY    = 'Segmentação';
	const EVENT_NAME        = 'Criar Segmentacao';
	...
	/**
	 * PostSegmentationNewHandler constructor.
	...
	public function build(Request $request, $extra = null)
	{
		...
		$mapperFields    = $mapper['routes']['segmentation_new']['fields'];
		$postFields      = $request->request->all();
        $eventProperties = [];

		foreach ($mapperFields as $key=>$event) {
			$value = $this->getFieldValueByKey($postFields['wspot_segmentation'], $key);
	...
	{
		return $this->convertSegmentationType($fields[$key]);
	}

	private function convertSegmentationType($type)
	{
		$types = [
			"registrations"     => "Cadastros",
			"visits"            => "Visitas",
			"uniqueGuests"      => "Visitantes Únicos",
			"recurringGuests"   => "Visitantes Recorrentes",
