...
class GetAdminRecordsPerDayReportHandler implements AnalyticsHandler
{
	const EVENT_CATEGORY    = 'Relatórios';
	const EVENT_NAME        = 'Cadastros Por Dia';
	...
	/**
	 * GetAdminRecordsPerDayReportHandler constructor.
	...
	public function build(Request $request, $extra = null)
	{
		...
		$mapperFields    = $mapper['routes']['admin_records_per_day_report']['fields'];
	...
	{
		if (strpos($requestUri, 'dateFromToWithLimitFilter') === false) return [];
		$explode = explode('?', $requestUri);
		parse_str($explode[1], $params);
		return $params['dateFromToWithLimitFilter'];
	}
}
