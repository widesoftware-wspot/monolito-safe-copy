...
class GetAdminDownloadUploadReportHandler implements AnalyticsHandler
{
	const EVENT_CATEGORY    = 'Relatórios';
	const EVENT_NAME        = 'Download Upload';
	...
	/**
	 * GetAdminDownloadUploadReportHandler constructor.
	...
	public function build(Request $request, $extra = null)
	{
		...
		foreach ($filters as $key=>$value) {
			$value = $this->translateFilter($key);
			if (!$value) return;
            $eventProperties['Filtrar'] = $value;
        }
	...
	{
		if (strpos($requestUri, 'downloadUploadFilter') === false) return [];
		$explode = explode('?', $requestUri);
		parse_str($explode[1], $params);
		return $params['downloadUploadFilter'];
	}
	...
	private function translateFilter($value)
	{
		...
		if (!array_key_exists($value, $filter)) return null;
