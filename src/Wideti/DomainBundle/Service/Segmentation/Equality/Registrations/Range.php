...
namespace Wideti\DomainBundle\Service\Segmentation\Equality\Registrations;
...
class Range implements Equality
{
    ...
    public function search(Filter $filter, FilterItem $filterItem, $isPreview = false, $params = [])
    {
        ...
        if ($filter->getType() == Filter::TYPE_ALL) {
            $guests = $this->guestRepository->segmentFindByCreatedRange($filterItem->getValue(), $filter->getIds(), $params);
            ...
            $guests = $this->guestRepository->segmentFindByCreatedRange($filterItem->getValue(), [], $params);
        }
