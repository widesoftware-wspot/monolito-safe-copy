...
namespace Wideti\DomainBundle\Service\Segmentation\Equality\Birthdays;
...
class Range implements Equality
{
    ...
    public function search(Filter $filter, FilterItem $filterItem, $isPreview = false, $params = [])
    {
        ...
        if ($filter->getType() == Filter::TYPE_ALL) {
            $guests = $this->guestRepository->segmentFindByDataNascimentoRange($filterItem->getValue(), $filter->getIds());
            ...
            $guests = $this->guestRepository->segmentFindByDataNascimentoRange($filterItem->getValue());
        }
        ...
        if (!$isPreview) {
            ...
            $guests = $this->guestRepository->findByIds($guestIds);
            return $guests;
        }
