...
class ApiEgoiType extends AbstractType
{
    ...
    public function checkFields($data, ExecutionContextInterface $context)
    {
	    ...
	    $id         = $this->session->get('apiEgoiId');
	    $entities   = $this->em
		    ->getRepository('DomainBundle:ApiEgoi')
		    ->findBy(['client' => $client]);

	    if (
            ($data->getInAccessPoints() == 0 && $this->apiEgoiService->checkAccessPointAlreadyExists($client, $id, null)) ||
            ($data->getInAccessPoints() == 1 && $this->apiEgoiService->checkAccessPointAlreadyExists($client, $id, null))
        ) {
            return $context
                ->buildViolation(
                    'Já existe uma integração criada com Todos Pontos de Acesso vinculados'
                )
                ->atPath('wspot_api_egoi_accessPoints')
                ->addViolation();
        }
        ...
        foreach ($data->getAccessPoints() as $ap) {
            if ($this->apiEgoiService->checkAccessPointAlreadyExists($client, $id, $ap)) {
                return $context
                    ->buildViolation(
